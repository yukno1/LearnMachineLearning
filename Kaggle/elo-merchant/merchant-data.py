...
merchant = pd.read_csv('../data/competitions/elo-merchant-category-recommendation/merchants.csv')
history_transaction = pd.read_csv(
    '../data/competitions/elo-merchant-category-recommendation/historical_transactions.csv')
new_transaction = pd.read_csv('../data/competitions/elo-merchant-category-recommendation/new_merchant_transactions.csv')
...
# 5、对于离散字段的缺失值使用平均值进行填充，后续有需要再进行优化处理。
...
transaction[category_cols] = transaction[category_cols].fillna(-1).astype(int)
